...
class ManageFaculity extends StatefulWidget {
  const ManageFaculity({super.key});

  @override
  State<ManageFaculity> createState() => _ManageFaculityState();
}

class _ManageFaculityState extends State<ManageFaculity> {
  ...
  try {
    final querySnapshot =
        await FirebaseFirestore.instance.collection('faculity').get();
  ...
  Future<void> _storeUserData(String userId) async {
    try {
      ...
      await firestore.collection('faculity').doc(userId).set({
  ...
  Future<void> _uploadImage(String userId) async {
    try {
      if (_selectedImage != null) {
        Reference ref =
            FirebaseStorage.instance.ref().child('faculity_images/$userId.jpg');
        ...
        await FirebaseFirestore.instance.collection('faculity').doc(userId).update({
          'imageUrl': imageUrl,
        });
      }
    } catch (e) {
  ...
  Future<void> _deleteHOD(String id) async {
    try {
      await FirebaseFirestore.instance.collection('faculity').doc(id).delete();
      setState(() {
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton.icon(
              ...
              label: Text(_isFormVisible ? "Cancel" : "Add Faculity"),
            ),
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(
                        labelText: "Faculity Name",
                        border: OutlineInputBorder(),
                      ),
                      validator: (value) {
                        if (value!.isEmpty) {
                          return "Please enter Faculity name";
                        }
                        return null;
                      },
                    ),
                    ...
                    ElevatedButton(
                      ...
                      child: const Text("Register Faculity"),
                    ),
                  ],
                ),
              ),
